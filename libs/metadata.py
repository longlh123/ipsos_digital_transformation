...
class Metadata(mrDataFileDsc):
    ...
    def addScript(self, question_name, syntax, is_defined_list=False, childnodes = list()):
        self.openMDM()
...
class SPSSObject_Dataframe(mrDataFileDsc):
    ...
    def generate_variable(self, field):
        if str(field.ObjectTypeValue) == objectTypeConstants.mtVariable.value:
            if field.DataType in [ dataTypeConstants.mtLong.value, dataTypeConstants.mtDouble.value, dataTypeConstants.mtText.value, dataTypeConstants.mtBoolean.value ]:
                self.transform_simple_data(field)
            elif field.DataType == dataTypeConstants.mtCategorical.value:
                self.transform_categorical_data(field)
        else:
            ...
            for f in field.Fields:
                if f.Properties["py_isHidden"] is None or f.Properties["py_isHidden"] == False:
                    self.generate_variables(f, iterations_list=iterations_list)

    def generate_variables(self, field, iterations_list=list()):
        for variable in field.Variables:
            if ".." not in variable.Indexes.split(','):
                variable_name = re.sub(pattern="[\{\}]", repl="", string=variable.Indexes)

                if str(variable.ObjectTypeValue) == objectTypeConstants.mtRoutingItems.value:
                    if variable.DataType in [ dataTypeConstants.mtLong.value, dataTypeConstants.mtDouble.value, dataTypeConstants.mtText.value, dataTypeConstants.mtBoolean.value ]:
                        self.transform_simple_data(variable)
                    elif variable.DataType == dataTypeConstants.mtCategorical.value:
                        #self.transform_categorical_data(variable, group_name=None if variable_name in iterations_list else "_97")
                        self.transform_categorical_data(variable, group_name=None)
                else:
                    self.generate_variable(variable)

    def transform_simple_data(self, field):
        var_name = self.get_variable_name(field)
        var_label = self.replaceLabel(self.get_variable_label(field))
    ...
    def transform_categorical_data(self, field, group_name=None):

        if group_name == "_97":
            a = ""

        var_name = self.get_variable_name(field, group_name=group_name)
        var_label = self.replaceLabel(self.get_variable_label(field, group_name=group_name))
        ...
        if field.Properties["py_showPunchingData"]:
            df_updated = self.df[field.FullName].replace(to_replace="[\{\}]", value="", regex=True)
    ...
    def get_categories_list(self, field, show_punching_data=False):
        ...
        if show_punching_data:
            ...
            categories_list[2] = "No".encode('utf-8')
        else: 
            for cat in field.Categories:
    ...
    def get_datatype(self, field):
        match field.DataType:
            case dataTypeConstants.mtLong.value | dataTypeConstants.mtDouble.value:
                return 0
            case dataTypeConstants.mtText.value:
                return int(self.df[field.FullName].astype(bytes).str.len().max())
            case dataTypeConstants.mtDate.value:
                return 0
            case _:
                return 0
            
    def get_variable_name(self, field, group_name=None):
        if str(field.ObjectTypeValue) == objectTypeConstants.mtVariable.value:
            ...
            if group_name is not None:
                indexes[len(indexes) - 1] = group_name
                
            iteration_name = self.get_iteration_name(field.Parent.Parent, indexes=indexes)

            var_name = "{}{}".format(var_name, iteration_name)

            return var_name

    def get_iteration_name(self, field, indexes=list()):
    ...
    def get_variable_label(self, field, group_name=None):
        if str(field.ObjectTypeValue) == objectTypeConstants.mtVariable.value:
            ...
            if group_name is None:
                ...
                var_label = "{} - {}".format(field.Label, self.get_iteration_label(field.Parent.Parent, indexes=indexes)) 
            else:
                var_label = "{} - {}".format(field.Label, "Khác") 
            
            return var_label

    def get_iteration_label(self, field, indexes=list()):
        if field.Parent.Parent is None:
            return field.Categories[indexes[field.LevelDepth - 1]].Label 
        else:
            return "{}_{}".format(field.Categories[indexes[field.LevelDepth - 1]].Label, self.get_iteration_name(field.Parent.Parent, indexes=indexes))
